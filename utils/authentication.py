...
def authenticate(event, token_type, jwt_secret, conn):
    ...
    query = f"select {token_type}_token from restaverse.users where client_id = {decoded_token['clientId']}" \
            f" and lower(email) = '{decoded_token['emailId'].lower()}'"
...
def update_jwt_tokens(conn, email, client_id, access_token_secret, refresh_token_secret):
    ...
    sql = f"UPDATE restaverse.users SET modified_on_utc = '{datetime.utcnow()}'," \
