...
def lambda_handler(event, context):
    ...
    query = f'select count(secret) from sys.sessions where user_id = {user_id}'
    ...
    session_count = result[0]['count']

    query = f'SELECT count(question_id) FROM sys.questions JOIN sys.sessions on sessions.session_id = ' \
            f'questions.session_id WHERE sessions.user_id = {user_id} '
    ...
    deets = {'sessions': session_count,
             'question': question_count}

    return api_response.generate_response(status_code=200, response_body=deets)
