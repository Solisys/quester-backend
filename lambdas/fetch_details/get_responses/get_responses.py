...
import get_responses_helper as helper
...
def lambda_handler(event, context):
    ...
    for question in questions:
        ...
        query = f'select users.name, responses.user_answer, responses.correct_answer, responses.time from ' \
                f'sys.responses join sys.questions on questions.question_id = responses.question_id join sys.users on ' \
                f'users.user_id = responses.user_id where questions.session_id = {session_id} and ' \
                f'questions.question_id = {question_id}'
        ...
        data.append({'description': question['description'], 'responses': responses})

    return api_response.generate_response(status_code=200, response_body={'data': data})
